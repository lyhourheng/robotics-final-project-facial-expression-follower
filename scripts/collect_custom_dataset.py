...
Collects facial expression images using YOLOv8-face detection.
"""
...
class EmotionDataCollector:
    def __init__(self, output_dir='datasets/custom_emotions', target_size=224):
        ...
        print("Controls: SPACE=Capture | 1-5=Switch emotion | Q=Quit")
    ...
    def _setup_directories(self):
        ...
        for emotion in self.emotions:
            ...
            existing = len([f for f in os.listdir(emotion_dir) if f.endswith('.jpg')])
            self.captured_counts[emotion] = existing
    
    def _preprocess_face(self, frame, face_coords):
        """Extract and resize face region for YOLOv8-cls."""
        x, y, w, h = face_coords
    ...
    def _save_image(self, image, emotion):
        """Save image to emotion folder."""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        filename = f"custom_{emotion}_{timestamp}_{self.captured_counts[emotion]:04d}.jpg"
        filepath = os.path.join(self.output_dir, emotion, filename)
        cv2.imwrite(filepath, image)
        self.captured_counts[emotion] += 1
        return filename
    
    def _preprocess_for_yolo(self, frame):
        """Preprocess image for YOLOv8 face detection."""
        img = cv2.resize(frame, (self.face_input_size, self.face_input_size))
    ...
    def _detect_faces_yolo(self, frame):
        ...
        input_blob = self._preprocess_for_yolo(frame)
    ...
    def collect(self):
        ...
        print(f"\n✓ Camera ready | Saving {self.target_size}×{self.target_size} images")
        ...
        while True:
            ...
            if key == ord('q') or key == 27:  # Q or ESC
                ...
                if len(faces) > 0:
                    face_resized = self._preprocess_face(frame, faces[0])
                    filename = self._save_image(face_resized, self.current_emotion)
...
if __name__ == '__main__':
    collector = EmotionDataCollector(
        output_dir='datasets/custom_emotions',
        target_size=224
    )
    collector.collect()
